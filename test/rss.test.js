...
describe('RSS feed contents', () => {
  it('renders Parashat ha-Shavua with the summary, Torah and Haftarah', async () => {
    const response = await request(server)
...
<description><![CDATA[<p>Tazria (“She Bears Seed”) opens by describing the purification process for a woman after childbirth. It then describes different forms of tzaraat, a discoloration condition on skin or clothing, and the requirement of an infected person to dwell alone outside the camp and be inspected by a priest.</p>
<p>Metzora opens by describing the purification process and accompanying sacrifices for one infected with tzaraat, a discoloration condition on the skin. It then describes the process of treating a house infected with tzaraat and the ritual impurity generated by certain bodily discharges.</p>
<p>Torah: Leviticus 12:1-15:33; Numbers 28:9-15</p>
...
<pubDate>-</pubDate>
  ...
  it('gives a doubled parsha one paragraph per portion', async () => {
    const response = await request(server)
        .get('/sedrot/index-en.xml?dt=2026-04-15');
    expect(response.status).toBe(200);
    // Tazria-Metzora: two summaries, not one run-on paragraph
    expect(response.text).toContain('by a priest.</p>\n<p>Metzora opens by');
  });

  it('leads a single parsha with its own summary', async () => {
    const response = await request(server)
        .get('/sedrot/index-en.xml?dt=2026-08-01');
    expect(response.status).toBe(200);
