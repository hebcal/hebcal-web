...
import {createMemo, applyIcalMemos} from '../src/icalMemo.js';
import {cloneEventWithMemo} from '../src/cloneEvent.js';
...
describe('createMemo', () => {
  ...
  it('appends the URL to an existing memo', () => {
    ...
    const erevPesach = cloneEventWithMemo(events[0], memo);
  ...
  it('combines memo, Torah reading and URL', () => {
    ...
    const events = HebrewCalendar.calendar(options);
    const pesachII = cloneEventWithMemo(events[2], 'Passover, the Feast of Unleavened Bread');
...
describe('applyIcalMemos', () => {
  it('never mutates the shared events returned by @hebcal/core', () => {
    ...
    const events = HebrewCalendar.calendar(options);
    const [orig] = events;
    const [copy] = applyIcalMemos(events, options);
    expect(copy).not.toBe(orig);
    expect(orig.memo).toBe(undefined);
    expect(copy.memo).toContain('Eighth Day of Assembly');
    expect(copy.getDesc()).toBe(orig.getDesc());
    expect(copy.getFlags()).toBe(orig.getFlags());
    expect(copy.render('en')).toBe(orig.render('en'));
  ...
  it('returns the event as-is when the memo is unchanged', () => {
    const hd = new HDate(new Date(2021, 1, 13));
    const ev = new Event(hd, 'User Event', flags.USER_EVENT);
    ev.memo = 'Unchanged';
    expect(applyIcalMemos([ev], {})[0]).toBe(ev);
  });
});
...
describe('DESCRIPTION generated by @hebcal/icalendar', () => {
  it('escapes newlines and folds long lines', () => {
    const options = {year: 1993, month: 4, sedrot: true, noHolidays: true};
    const events = HebrewCalendar.calendar(options);
    const [ev] = applyIcalMemos(events, {...options, dtstamp: 'X'});
    const lines = new IcalEvent(ev, {...options, dtstamp: 'X'}).toString().split('\r\n');
  ...
  it('omits DESCRIPTION when there is no memo', () => {
    ...
    ev.alarm = false;
    const [ev1] = applyIcalMemos([ev], {});
    const lines = new IcalEvent(ev1, {dtstamp: 'X'}).getLongLines();
